...
Project Suzu 是一个用 Rust 编写、面向原创短篇视觉小说的脚本优先 VN 框架。新项目可以从 `templates/starter-vn` 开始，并通过 `suzu-player` 直接运行，不需要先写 Rust 入口。
...
- 零代码工程入口：`suzu-player` 可直接打开带 `game.suzu.toml` 的脚本优先工程目录。
...
作者入口推荐使用 `suzu-player`、`templates/starter-vn`、`scenario/` 脚本目录和 `assets/` 资源目录。新手写脚本优先使用 `syntax=indent`；`syntax=classic` 保留为兼容语法，`syntax=braces` 和 `syntax=markup` 主要面向生成器、导入导出和结构化工具。

迁移工具仍处于预览阶段：明文 XP3 资源读取、XP3 Viewer 和有限 KRKR package scan mode 仅用于授权资产检查和迁移实验，不是 Project Suzu 的主制作路径。项目方可以为自己有权处理的资源包提供外部 XP3 plugin，但游戏专用处理器不放入本仓库。支持边界见：[docs/xp3-support.md](docs/xp3-support.md)、[docs/xp3-plugin-interface.md](docs/xp3-plugin-interface.md)、[LEGAL.md](LEGAL.md) 和 [SECURITY.md](SECURITY.md)。

## 下载使用

打开 GitHub Release：

[https://github.com/fangbm/ProjectSuzu/releases/tag/v0.2.1](https://github.com/fangbm/ProjectSuzu/releases/tag/v0.2.1)

按系统下载：

- Windows: `project-suzu-v0.2.1-windows-x64.tar.gz`
- Linux: `project-suzu-v0.2.1-linux-x64.tar.gz`
...
快速上手见：[docs/getting-started.zh-CN.md](docs/getting-started.zh-CN.md)。完整框架使用指南见：[docs/framework-guide.zh-CN.md](docs/framework-guide.zh-CN.md)。工程目录规范见：[docs/project-layout.zh-CN.md](docs/project-layout.zh-CN.md)。低门槛模板见：[templates/starter-vn/README.zh-CN.md](templates/starter-vn/README.zh-CN.md)，Rust 集成模板见：[templates/minimal-vn/README.zh-CN.md](templates/minimal-vn/README.zh-CN.md)。短篇 VN demo 计划见：[docs/short-vn-demo.zh-CN.md](docs/short-vn-demo.zh-CN.md)。API 稳定性说明见：[docs/api-stability.zh-CN.md](docs/api-stability.zh-CN.md)。脚本参考见：[docs/scripting-reference.zh-CN.md](docs/scripting-reference.zh-CN.md)。XP3 插件接口见：[docs/xp3-plugin-interface.zh-CN.md](docs/xp3-plugin-interface.zh-CN.md)。第三方依赖许可证原文见：[THIRD_PARTY_LICENSES.md](THIRD_PARTY_LICENSES.md)，中文导读见：[THIRD_PARTY_LICENSES.zh-CN.md](THIRD_PARTY_LICENSES.zh-CN.md)。品牌素材边界见：[assets/branding/README.zh-CN.md](assets/branding/README.zh-CN.md)。

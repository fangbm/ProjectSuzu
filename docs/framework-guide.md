...
Project Suzu 仍处于 `0.2.x` 阶段。运行时、脚本格式和工具链已经能支撑小型项目和实验性迁移工作，但 API 仍可能在后续版本中收敛。需要兼容性判断时，以 `docs/api-stability.md` 为准。
...
- `suzu-project`：读取 `game.suzu.toml` 和约定目录，创建可直接运行的工程。
...
1. 作者维护 `game.suzu.toml`、`scenario/main.szs` 和 `assets/`。
2. `suzu-player` 调用 `suzu-project` 读取配置、扫描资源、加载入口脚本。
3. `SuzuApp` 推进脚本、动画、文本、音频和 UI 状态。
4. 平台层把 `DesktopFrame` 渲染到窗口。
...
```powershell
cargo run -p suzu-player -- my-novel
cargo run -p suzu-player -- --check my-novel
```

`game.suzu.toml` 和资源 ID 规则见 `docs/project-layout.md`。如果目录没有配置文件，`suzu-player` 会按约定尝试 `scenario/main.szs`，再兼容旧模板的 `script/main.szs`。

如果你需要自定义 Rust 入口，才需要新增 Rust package。workspace 内项目可以参考：
...
普通作者可以跳过本节，直接使用 `suzu-player`。本节适合需要自定义窗口、平台集成或 Rust 侧业务逻辑的工程。
...
```rust
app.register_package_file("target/hello-world.suzupack")?;
```

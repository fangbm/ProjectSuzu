...
Project Suzu 是一个 Rust 视觉小说框架。它提供脚本编译器、低门槛工程运行器、保留式场景模型、文本 reveal 和历史记录、存档/读档状态、音频状态同步、资源打包和桌面示例。
...
- `crates/suzu-project`：`game.suzu.toml` 和约定目录工程加载。
...
- `tools/suzu-player`：标准 Suzu 工程文件夹的零代码运行器。
- `tools/suzu-launcher`：统一桌面入口，用于打开 Suzu 项目、导入 XP3 archive 和运行 preview。
- `tools/suzu-xp3-viewer`：桌面 XP3 检查和图片/文本预览工具。

## 运行时流程

1. `suzu-player` 或 `suzu-launcher` 读取 `game.suzu.toml`，然后按约定回退到 `scenario/main.szs` 或 `script/main.szs`。
2. 工程加载器注册散文件资源、package archive 和入口脚本。
3. 将 `.szs` source 解析为 `ScriptDocument`。Parser 会从脚本头检测 `syntax=classic`、`syntax=indent`、`syntax=braces` 或 `syntax=markup`。
4. 将 document 编译为 VM commands，并交给 `SuzuApp`。
5. 如果 `GameConfig.title_screen.enabled` 为 true，则先显示标题界面。
6. 用 frame delta 和 input event 推进 app、渲染场景，并通过 save manager 捕获存档。
